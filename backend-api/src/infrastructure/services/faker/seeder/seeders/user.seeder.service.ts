...
export class UserSeederService {
  ...
  async seed(): Promise<void> {
    ...
    const fakeModeratorAuth0Id = ['auth0|661f88a9992913311d1add5'];

    for (const auth0Id of fakeArtistAuth0Id) {
    ...
    for (const auth0Id of fakeUserAuth0Id) {
      if (await this.userRepository.findOneBy({ auth0Id: auth0Id })) {
        continue;
      }
      const user = new User();
    ...
    for (const auth0Id of fakeModeratorAuth0Id) {
      if (await this.userRepository.findOneBy({ auth0Id: auth0Id })) {
        continue;
      }
      const user = new User();
