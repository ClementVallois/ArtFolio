<template>
            ...
            <div class="absolute top-0 right-0 mt-2 mr-2" v-if="props.myProfile && !props.postIsPinned"
                ...
                <div class="bg-white rounded p-1 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        class="bi bi-three-dots-vertical text-black" viewBox="0 0 16 16">
                        <path
                ...
                <div class="absolute top-8 right-0 bg-white rounded shadow-lg w-[15rem]" v-show="dropdownOpen">
                    <ul class="flex w-[100%] justify-center">
                        <li class="py-2 px-4 hover:bg-gray-200 cursor-pointer text-black text-sm" @click="openModal">
                            Supprimer la publication</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <ModalDeleteComponent v-if="showModal" @closeModals="closeModal" @deleteData="handleDelete"
        textModal="Voulez-vous vraiment supprimer cette publication ?"></ModalDeleteComponent>
    <AlertComponent v-if="showAlert" v-model:alertError="alertError" @closeAlert="handleCloseAlert"
        textAlert="Votre post vient d'être supprimer"></AlertComponent>
