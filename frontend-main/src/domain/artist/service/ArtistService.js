...
function artistService() {
  ...
  async function modifyArtist(id, data) {
    try {
      ...
      storeGlobal.logError(
        "Erreur lors de la modification des informations d'un artiste : " + error.message,
        6
      )
      throw new Error(error.message)
    }
  ...
  async function createPersonalDataRequest(id) {
    try {
      return apiArtist.createPersonalDataRequest(id)
    } catch (error) {
      storeGlobal.logError(
  ...
  async function getLastRegisteredArtist(number) {
    try {
      ...
      storeGlobal.logError(
        'Erreur lors de la récupération des derniers artistes inscrits: ' + error,
        6
      )
    }
  }
  ...
  async function getRandomArtist(number) {
    try {
      ...
      storeGlobal.logError(
        'Erreur lors de la récupération des derniers artistes aléatoires : ' + error,
        6
      )
    }
  }
  ...
  async function getArtistProfilePicture(id) {
    try {
      ...
      storeGlobal.logError(
        "Erreur lors de la recuperation d'une photo de profil' : " + error.message,
        6
      )
      throw new Error(error.message)
    }
