<template>
    ...
    <NavUserComponent v-else-if="globalStore.profile?.role === 'user'" />
    <NavComponent v-else />
...
watch(globalStore.profile?.value, (newValue) => {
    console.log('globalstore.profile?.Value.role', globalStore.profile?.Value.role)
})
